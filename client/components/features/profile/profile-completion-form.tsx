...
import type { CreateProfileParams } from '@/services/features/profile/profile.types';

interface ProfileCompletionFormProps {
  readonly userId: string;
}

export function ProfileCompletionForm({ userId }: ProfileCompletionFormProps) {
  const t = useTranslations('profile.complete.form');
  const tValidation = useTranslations('profile.complete.validation');
  const router = useRouter();
  const createMutation = profileQueries.useCreateProfile();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await createMutation.mutateAsync({ userId, params });
